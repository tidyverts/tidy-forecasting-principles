...
UKLungDeaths %>% 
  ...
  components
```
```{r components-display}
mdeaths_components <- UKLungDeaths %>% 
  ETS(mdeaths) %>% 
  components
mdeaths_components
mdeaths_components %>% 
...
For example, the `fpp2::mens400` dataset contains Olympic men's 400m track final winning times. The winning times for the 1916, 1940 and 1944 Olympics are missing from the dataset.

```{r mens400, echo = FALSE}
as_tsibble(fpp2::mens400) %>% 
  ggplot(aes(x = index, y = value)) + 
  geom_line() + 
  theme_minimal()
```
...
mens400_complete <- as_tsibble(fpp2::mens400) %>% 
  LM(value ~ trend()) %>% 
  interpolate()
mens400_complete
...
mens400_complete %>% 
  ggplot(aes(x = index, y = value)) + 
  geom_line(aes(colour = "Interpolated")) + 
  geom_line(aes(colour = "Actual"), data = as_tsibble(fpp2::mens400)) + 
  theme_minimal() + 
...
Suppose we are using an ETS model to estimate the `mdeaths` series:
```{r ets-mdeaths}
library(fable)
mdeaths_ets <- as_tsibble(mdeaths) %>% 
  ETS(value)
```

We may be interested in using the same model with the same coefficients to estimate the `fdeaths` series:
```{r ets-refit}
refit(mdeaths_ets, as_tsibble(fdeaths))
```
